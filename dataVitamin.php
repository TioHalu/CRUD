...
session_start();
    ...
    if(isset($_GET['detail'])) {
        ...
        $vitamins = mysqli_query($connect, "SELECT * FROM vitamin WHERE user_id=$id") or die($connect->error());
        if(isset($result)) {
    ...
    if(isset($_POST['save'])) {
        ...
        $result = $connect->query("INSERT INTO vitamin (user_id, tanggal, jenis_vit) 
                        VALUES('$id', '$tgl', '$jenis')") 
        ...
        header("location: dataVitamin.php?detail=$id");
    }
    if(isset($_GET['delete'])) {
        $id_data = $_GET['delete'];
        $result = $connect->query("DELETE FROM vitamin WHERE id=$id_data") or die ($mysqli->error());
        if($result) {
...
<?=template_header('Detail Vitamin')?>
...
<!-- card -->
<div class="card mt-5">
    <h5 class="card-header ">Data Vitamin</h5>
    <div class="card-body ">
        <div class="row">
            <div class="col-2">
                <label for="">NIK</label>
        ...
        <table class="table table-striped text-center">
            ...
            <tbody>
                <?php while($row = mysqli_fetch_array($vitamins)){?>
                <tr>
                    <th scope="row"><?php echo $row['tanggal']?></th>
                    <td><?php echo $row['jenis_vit']?></td>
                    <td><a href="dataVitamin.php?detail=<?=$id?>&delete=<?php echo $row['id']?>" class="btn btn-danger" style="background: red">Hapus</a></td>
                </tr>
                <?php } ?>
            </tbody>
        </table>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="staticBackdropLabel">Vitamin</h5>
